...
| **DAC topology constraint** | The DAC mesh is a star centered on 9975WX (M3, Proxmox, TrueNAS each have one DAC link to 9975WX; no direct Proxmox↔TrueNAS DAC). DNS-from-PiHole reaches the Proxmox-hosted Registry via the LAN bridge (fine — DNS payloads are tiny). Plugin/MCP pull traffic from compute hosts to Proxmox-hosted Registry: **9975WX uses the direct DAC link** (`10.60.2.0/24`); **M3 uses the GB Local LAN** (no direct DAC path from M3 to Proxmox — M3's single DAC link goes to 9975WX only). LAN-speed plugin pulls from M3 are operationally fine because plugin manifests and MCP invocation payloads are small; DAC bandwidth matters more for heavy data transfer (ClickHouse query traffic, ZFS replication) which doesn't pass through PCS-Registry. Worth being explicit because the per-host network path affects where any future bottleneck could surface. |
