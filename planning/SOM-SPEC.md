...
**SOM-OQ-7**: **Context-entropy / signal-dilution at the AKB retrieval surface** (per Einstein finding #4 of the 2026-06-04 coherence pass, Patton adjudication `f43fff7f`). The corpus governs retrieved-context **volume** (token-bounded role projection + retrieval size limits per AKB's spec) but does NOT govern retrieved-context **signal quality under dilution** — the needle-in-a-haystack decay where injecting more role-projected AKB context raises prompt entropy and degrades the attention mechanism's ability to lock onto the relevant vector. Verified novel against the corpus: AKB's retrieval contract + role-projection budget are byte/token-bounded; nothing addresses retrieval *relevance-floor under dilution*. **Scope question (Judge's call):** is this a **spec-level** open question (commit to a relevance-floor invariant — "retrieval must govern signal-to-noise, not just volume") OR a **build-time tuning item** (empirical retrieval-ranking + projection-strategy tuning that varies per model / per workload, no structural mesh invariant)? Naming it here so it is NOT silently assumed-away regardless of which path Judge selects; if spec-level, this OQ promotes to an AKB-spec invariant; if build-time, it lands in `SOM-ENGINEERING-STANDARDS.md` as a tuning discipline. Either resolution requires the AKB spec corpus (currently in `ionis-devel/planning/akb-*.md`, migrating to `som-spec/planning/akb/` per `REPO-SHAPE-DECISIONS.md` v0.3 § Where Specs Live) to declare its position. **Why this couldn't be a Watson-or-Bob finding**: agents reasoning natively on attention/entropy as a system property (rather than as an empirical tuning concern) is the substrate-independence Einstein brought — it bites at scale (large AKB corpus + multiple concurrent role projections) and is the kind of property a different substrate catches that Claude-internal review tends to assume-away.
