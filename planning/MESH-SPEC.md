---
title: "the mesh Spec — Fiducial Mesh Integrative Capstone"
doc_type: spec
status: validated
version: v1.0
authors:
...
- Does NOT introduce new pillars. The mesh is the eight named pillars; future pillar additions follow PILLAR-NAMES discipline + Judge sign-off, not this spec.
...
**Plus the cross-cutting concurrency design** (`CONCURRENCY-AND-ARCHETYPES.md`, PR #62) — not a pillar, but a load-bearing cross-cutting contract referenced by IBX v1.0 + IAM v1.0 + DPG v1.0 + CRB v1.0.

### Validation status (where each pillar sits on the design-vs-built line)

This is a structural property of the mesh: every pillar's *contract* is now committed at v1.0; every pillar's *implementation* is at a known maturity level. The honest framing per `PRODUCTION-VALIDATION.md` v1.1:
...
Per `MANIFESTO.md` v0.6 §0 + `TECHNICAL-OVERVIEW.md` v0.2. The mesh has three architecturally distinct planes:
...
Per `MANIFESTO.md` v0.6 §0: the dotted line between Issuance and Control is **a deliberate security property, not tidiness**. Because ARCA is never in the action path, it can be kept offline; an offline authority cannot be attacked over the network during operation. Runtime verification is local (signature + trust chain), never a callback. The air-gap is the design assumption; every pillar's runtime behavior respects this separation.

## Three-Layer Deployment Model

Above the three internal planes (Issuance / Control / Workforce, which structure the *mesh* itself), the mesh-as-a-deployed-product has three external **layers** that the customer experiences:
...
- **Substrate** (bottom) is what the customer provides per the substrate-substitutability invariant `MI-8` + its mesh-level enforcement `CD9` + the conformance mechanics `CONF-CD1..11`. Per-seam, per-tier conformance; the mesh ships a reference connector but the customer chooses + operates the backend.
...
This section names the **exact substrate profile set** MI-8 substitutability is defined against. The set is **the boundary of the substitutability claim, not an illustration of a broader one**: a deployment that swaps in a substrate listed here is covered by the multi-profile conformance run (per CD15 + CONF-CD1..11); a substrate outside the set is a **new conformance run, not a covered claim**. "Substitutable" without naming the set is the overclaim Einstein's coherence pass surfaced; this table is the fence.
...
| **Telemetry sink** (per MI-11) | OTel Collector → Tempo/Prometheus | Azure Monitor / App Insights | Datadog | open-stack vs MS-stack vs SaaS observability |

**The three-profile minimum is the conformance discipline, not a target ceiling.** Per CONF-CD7 (per-seam + cross-seam) and CONF-CD11 (pillar-owned profile content), the multi-profile run for each seam is the structural mechanism that catches a co-evolved core seam silently encoding a substrate-specific property: if the seam works on Profile A but breaks on Profile B or C, the off-profile failure surfaces at CI before merge. That is what makes the core monorepo's co-evolved seams (PCT, event enum, identity claims, etc., per REPO-SHAPE-DECISIONS § core) provably substrate-neutral rather than asserted-substrate-neutral.

**Adding to the set is a deliberate motion.** A future substrate added to a row (e.g., MongoDB at IBX, Postgres at AKB-without-pgvector, AWS Secrets Manager at IAM) requires: (1) a profile definition codifying that substrate's query/usage pattern per CONF-CD11, (2) a conformance suite extension to test the seams against it, and (3) the multi-profile run passing. Until those three land, the new substrate is **not** part of the MI-8 guarantee; consumers depending on it ship under their own evaluation. This is the same boundary discipline as `DELIVERY-PACKAGING.md` DP-CD's base-OS-scope wording (tested-on-RHEL-family-not-any-Linux) — bounded, named, honest.

## Mesh Control Center (MCC)

The **Mesh Control Center (MCC)** is the admin **control plane *over* The mesh** — the human single-pane that aggregates every pillar's seams. MCC is **not a 9th pillar**: The mesh stays at eight (IAM, IBX, PCS, ACT, AKB, CRB, PGE, DPG). MCC aggregates; pillars own logic.
...
This duality is structural: every pillar function is reachable via CLI/API/MCP (the agent surface), and MCC is a *client* of those same surfaces (the human surface). MCC is never privileged; it never bypasses a CLI command an agent could also run. The CLI-first/UI-second discipline (per `ENGINEERING-STANDARDS.md`) is what keeps MCC thin and prevents MCC from becoming a parallel-and-divergent control surface.
...
| Audit + Metering | ACT | Audit events (MI-1 stream), metering rollups (MI-11 stream) |
| Knowledge | AKB | Knowledge base browse + search |
| Telemetry | (embedded Grafana/Prometheus/Tempo) | Cross-pillar traces, metrics dashboards (MI-11) |
| Substrate + Conformance | (MI-8 + CONF-CD1..11) | Live substrate matrix, conformance attestation status per seam |
| Compute | CRB | Job state, worker-pool status (MI-7) |
...
- **Project**: `Mesh.Console` in `core` (per repo-shape decision — one .NET solution for all the mesh implementation).
...
- **Authentication**: MCC authenticates humans via IAM (federated to the customer's AD/Entra/IdP). MCC consumes the Identity seam exactly as agents do — same substrate-pluggable contract per MI-8 + CONF-CD1..11; the customer's IdP choice (AD / Entra / OpenLDAP / Keycloak) plugs in identically for human authentication and agent identity verification.

### What MCC is NOT

- **Not a 9th pillar** — adding MCC does not change the pillar count, the MI-10 invariant ("eight pillar contracts + the other twelve mesh invariants"), or the conformance scope. CD1 (eight pillars compose into one mesh) is unaffected.
...
| PGE → ACT (rule decision emission) | PGE v1.0 CD9 (pge.* events) | ACT v1.0 CD4 (bounded enum extension via VP) | ⏳ Composes via VP-1 |
...
| ACT cross-plane consumer | ACT v1.0 CD on ingest-from-all-pillars | All pillars emit `<pillar>.*` events | ⏳ Composes via VP-1 for four pillars |
...
**Result**: 17 of 19 enumerated seams compose cleanly at v1.0. 2 of 19 (PGE→ACT, ACT cross-plane four-pillar) compose via the shared curation event VP-1. No seam has a gap or double-coverage; CRB and DPG orthogonality is of *concerns*, not of *decisions* (concerns separate; decisions couple at workload level per the eligibility-input pattern); no pillar silently front-runs an Increment-2 ruling.
...
This pattern is mesh-level discipline that v1.0 the mesh commits. **No pillar's DR-IAM-4 coupling may defer the audit invariant OR the terminal-state resolution**; only runtime continuation is deferable. This rule applies to any future pillar that gains in-flight work.
...
This pattern is mesh-level discipline that v1.0 the mesh commits. **Future bootstrap-credential design (any pillar's daemon, including pillars not yet named) must select from the external-anchor set; in-mesh sources are excluded by CD13.**

## Campaign-Level Open Items (Mesh Tracking)

### VP-1: Four-pillar shared ACT v1.x curation event
...
**Resolution trigger**: when Bob's ACT Record Layer build reaches the curation-event-emission milestone, the single shared event closes VP-1 atomically across all four pillars.

### OQ-1: Post-merge forward-reference sweep
...
**Not blocking**: OQ-1 is **post-merge cosmetic**, not a v1.0 the mesh spec blocker. Per Patton's `ea67f2ac` close-out on PR #68 and `f346fdab` close-out on PR #69, this is sweep-on-next-touch discipline.

### OQ-2: PGE rule-lifecycle reconciliation (per Patton `f346fdab` flag)
...
**Forward note**: Patton recommends a second Einstein pass scoped to the wave-2 specs once the four-item tightening lands (`dc6ca481`). OQ-6 is the kind of seam-gap that benefits most from cross-substrate review and may surface adjacent gaps when the resolution is selected.

## Mesh-Level Invariants

Beyond the pillar-level CDs, v1.0 the mesh commits these **mesh invariants** that hold across all eight pillars:

**MI-1**: **Audit retention AND terminal-state resolution are non-negotiable across all pillars.** Every pillar with in-flight work emits ACT events for that work; the events are preserved regardless of session termination, runtime failure, or policy rejection. **Plus** (per Einstein finding #5, `dc6ca481`): every in-flight item reaches a *terminal* audit state — even when runtime continuation is deferred to a Judge ruling, the audit record gets a defined terminal marker (e.g., `runtime_continuation_deferred_pending_ruling`). Runtime continuation may be deferred; audit retention and terminal-state resolution may not. *Caveat — see OQ-6: between event-generation inside a hard-isolating boundary (e.g., DPG sandbox) and event-arrival at ACT ingest, an unprotected interval exists where boundary-local telemetry may be annihilated before reaching ACT. The retention guarantee applies to events that arrived at ACT; the in-boundary-to-ACT seam is the surface OQ-6 is tracking.*

**MI-2**: **PGE is the single source of policy truth across the mesh.** DPG, PCS-Daemon, CRB, and any future pillar that enforces policy at a chokepoint consume rules from PGE; none embed their own corpus. The mesh's policy surface is uniform; deviation is a mesh-conformance violation.

**MI-3**: **IAM is the single source of verified identity across the mesh.** PGE, IBX, PCS-Daemon, CRB, DPG, ACT consume verified identity from IAM; none mint or verify identity locally. The mesh's identity surface is uniform; deviation is a mesh-conformance violation.

**MI-4**: **The dotted line between Issuance and Control planes is observed by every pillar.** No pillar's runtime behavior depends on ARCA's online availability. Runtime identity verification is local (signature + trust chain), never a callback to the Issuance Plane.

**MI-5**: **The double-guardrail (PGE Gate 1 + Gate 2) is preserved across substrate substitutions.** A future substrate swap that centralizes enforcement at a single service (e.g., OPA cluster) must still apply both gates at distinct chokepoints. CD3 in PGE v1.0 is architectural commitment, not implementation detail.

**MI-6**: **Bounded event-type enums require explicit curation events for extension.** ACT v1.0 CD4 establishes the discipline; PCS / DPG / CRB / PGE all follow it via VP-1. Any future pillar that emits ACT events follows the same pattern.

**MI-7**: **Worker-pool dispatch (SKIP-LOCKED claim + lease/timeout + idempotency + mid-action-safe termination) is the canonical pattern for parallel workloads of a single identity.** DPG v1.0 CD7 commits this; future pillars with parallel-worker concerns follow.

**MI-8**: **Substrate substitutability is defined as passing the multi-profile conformance run, not asserted as a property.** A pillar is substitutable across exactly the profiles its seam contracts are tested against in § Tested Substrate Profiles — and no further. The mesh-level Exit Test is the conjunction of pillar-level conformance suite passes (CONF-CD1..11) against the named profile set; substrate choice in one pillar may not create lock-in for another, *because* the conformance run catches a co-evolved core seam that would silently encode a substrate-specific property. The substrate-neutrality of co-evolved core seams (PCT nine-field surface, bounded ACT event enum, identity claims, et al.) is what the multi-profile run **proves**, not what this spec **asserts**. A seam change that fails any profile in § Tested Substrate Profiles does not merge (CD15). This converts the build-time-coupling (monorepo) vs runtime-substitutability (this invariant) orthogonality from asserted-orthogonal to measured-orthogonal on every CI run.

**MI-9**: **The seven IAM Increment-2 rulings are the single deferral surface for Judge.** All ruling-pending behavior across pillars couples to one of the seven (or to IBX DR1 + DR-IAM-7 for ITDR). No pillar introduces an Increment-2 ruling outside the seven without Judge sign-off + PILLAR-NAMES update.

**MI-10**: **Mesh conformance is verifiable.** A deployment is conformant if and only if it satisfies all eight pillar contracts + the other twelve mesh invariants (MI-1..9 + MI-11..13). Conformance is testable; the success criteria below name the tests.

**MI-11**: **Telemetry contract — every pillar emits OTLP traces, OTLP metrics, and JSON-structured logs with identity/session/service attributes.** Every pillar's runtime behavior is observable via OpenTelemetry: OTLP traces for cross-pillar operations, OTLP metrics for runtime state (including the token + cost counters that ACT consumes for chargeback), JSON-structured logs to stderr (stdout is reserved for MCP protocol channel) with `timestamp`, `level`, `message`, `service.name`, `service.version`, `trace_id`, `span_id`, `identity`, `session` keys plus event-specific fields. The customer selects the **Telemetry-sink** — App Insights / Azure Monitor, OCI Monitoring, Datadog, Grafana/Prometheus/Tempo stack, etc. — and configures the OTLP endpoint via `OTEL_EXPORTER_OTLP_ENDPOINT`; the mesh does not name the sink. Definition-of-done weight equal to MI-1 (audit retention): a pillar without OTLP traces + OTLP metrics + structured-JSON logs is non-conformant. Distinct from MI-1: MI-1 governs *audit* telemetry (the durable accountability record); MI-11 governs *observability* telemetry (the operational + cost-attribution surface). ACT consumes both: audit events from the MI-1 stream, runtime metering from the MI-11 stream. Substrate-pluggability per MI-8 extends to the Telemetry-sink seam: the contract is OTLP-on-the-wire, never a specific backend.

**MI-12**: **Spec-Harness-Registry primitive — pluggable-artifact governance follows the three-part contract.** When a pillar governs a set of pluggable artifacts — substrate products (per MI-8 conformance), plugins (per PCS), and future content classes (AKB knowledge curation, PGE rule packs) — the governance follows a three-part contract: (1) a declarative **Spec** (what a valid artifact must satisfy), (2) a mechanical **Test Harness** (the conformance gate — non-conformant artifacts are rejected), (3) a **Registry** (the certified set — only Harness-passed artifacts enter, versioned per the artifact-interface SemVer). The primitive already exists at two mesh-level surfaces: **substrate conformance** (MI-8 expressed via `CONF-CD1..11`: the Spec is the seam contract, the Harness is the conformance suite, the Registry is the substrate matrix), and **PCS plugin governance** (PCS-Syntax = Spec, PCS-Daemon writes the Registry after Harness pass, PCS-Registry = certified set). Future pluggable-artifact governance — AKB knowledge classes, PGE rule packs — SHOULD instantiate this primitive rather than rolling bespoke governance. Same Harness and Registry implementations can be reused across pillars; the only per-pillar variation is the Spec content. This invariant is what makes mesh-wide spec-driven governance a reusable architectural pattern rather than a coincidence.
...
**Composition with MI-1** (per Einstein STRAIN #1 follow-up flag, `f43fff7f`): a **reconciliation-pending state is an audited in-progress state, not a violation of MI-1's terminal-state-resolution requirement**. The intermediate state is itself emitted to ACT (e.g., `iam.mint_pending`, `pcs.registry_write_pending`, `dpg.worker_state_pending`); MI-13's bounded sweep is *how* the MI-1 terminal state is reached for distributed writes — fully-applied or fully-rolled-back, both terminal, both audited (e.g., `iam.mint_committed` or `iam.mint_rolled_back`). MI-1 and MI-13 compose: MI-1 requires every in-flight item *reach* a terminal audit state; MI-13 commits the reconciliation sweep as the mechanism that reaches it for the multi-store class of writes. A reconciliation-pending event in the ACT trail is the audit record of "in progress, bounded by the sweep window," not a half-state that escapes MI-1.

## Closed Decisions (CDs — v1.0 Mesh Commitments)

**CD1**: **Eight pillars compose into one mesh** per the inventory and dependency graph above. No additional pillars at v1.0; pillar additions follow PILLAR-NAMES discipline + Judge sign-off.

**CD2**: **Three-plane architecture is structural** (Issuance + Control + Workforce). The dotted line between Issuance and Control is load-bearing security property.

**CD3**: **Seam composition is the mesh contract.** All cross-pillar couplings trace to committed surfaces on both sides (§ Seam Composition Verification). No gaps; no double-coverage; orthogonality where ruled (CRB ⊥ DPG per `251c9511`).

**CD4**: **Seven IAM Increment-2 rulings enumerated in ONE place** (§ Seven IAM Increment-2 Rulings). All pillar DRs that defer to a ruling cite the corresponding DR-IAM-N row. No silent front-running.

**CD5**: **The DR-IAM-4 audit-vs-runtime split is mesh-level discipline.** Audit invariant is never deferred. **Per Einstein finding #5 (`dc6ca481`) airtightening: terminal-state resolution is also never deferred** — every in-flight item reaches a terminal audit state even when runtime continuation is deferred (e.g., `runtime_continuation_deferred_pending_ruling`). Only runtime continuation is deferable. Applies to all current and future pillars.

**CD6**: **Four-pillar shared ACT v1.x curation event tracked at mesh level** (VP-1). Resolves four pillar VPs atomically; prevents half-extended-enum failure mode.

**CD7**: **Post-merge forward-reference sweep tracked at mesh level** (OQ-1). Sweep-on-next-touch discipline; not blocking.

**CD8**: **Thirteen mesh-level invariants** (§ Mesh-Level Invariants MI-1..13) hold across all pillars regardless of pillar version. Deviation is a mesh-conformance violation.

**CD9**: **Substrate substitutability is mesh-level**, not just pillar-level. MI-8: no pillar's substrate may create lock-in for another. The mesh-level Exit Test is the conjunction of pillar-level conformance suite passes against the profile set named in § Tested Substrate Profiles, enforced at CI by CD15.

**CD10**: **PGE is single source of policy truth (MI-2); IAM is single source of verified identity (MI-3); ACT is single source of cross-pillar audit (MI-1).** Three uniform-surface commitments that distinguish a mesh from a federation of independent pillars.

**CD11**: **Audit completeness is testable.** MI-1 + MI-6 + ACT v1.0 CD3 (append-only) compose: every action emits events; every event lands; the chain is verifiable.

**CD12**: **This spec is integrative, not derivative.** It does not re-derive pillar contracts; it asserts mesh-level composition. Each pillar's spec remains authoritative for that pillar; this spec is authoritative for the mesh.

**CD13** (added per Einstein finding #3, `dc6ca481`): **The DR-IAM-2 external-anchor base case invariant is mesh-level discipline.** Bootstrap credentials originate from outside the mesh — host-level token, hardware-bound key, or offline-ARCA-provisioned initial credential. No in-mesh authority may serve as bootstrap source. This invariant binds at v1.0; the specific mechanism choice remains deferred to DR-IAM-2 ruling, but the *admissible set* is bounded now. Any future ruling that selects an in-mesh source is structurally inadmissible.

**CD14**: **MCC (Mesh Control Center) is the human control plane, not a 9th pillar.** Per § Mesh Control Center (MCC): MCC aggregates pillar seams as panes for human operators; it owns no business logic, no policy enforcement, no identity verification, no audit storage. Pillar count stays at eight (CD1 unaffected); MCC is a *consumer surface* parallel to the agent-facing MCP protocol. The CLI-first/UI-second discipline (per `ENGINEERING-STANDARDS.md`) ensures every MCC pane reflects a CLI/API surface an agent could also call — MCC is never a privileged or parallel-and-divergent control surface. Implementation: web (ASP.NET Core + Blazor), `Mesh.Console` project in `core`. The `inbox-ui` prototype is the seed; production MCC grows from it.

**CD15** (added per Einstein STRAIN #1, `f43fff7f` + `b808eb5e`): **Co-evolved core seams are substrate-neutral by construction, enforced by the multi-profile conformance run — not by reviewer judgment.** Per § Tested Substrate Profiles + MI-8: a seam change in the `core` monorepo that fails any profile in the named profile set does not merge. This is the structural mechanism that makes the monorepo decision (REPO-SHAPE-DECISIONS § core) and the substitutability invariant (MI-8) provably compatible rather than asserted-compatible. The position is *"we are substitutable across exactly what we test, and here is what we test"* (§ Tested Substrate Profiles is the boundary of the claim, not an illustration). Adding a substrate to the set is a deliberate motion requiring a profile definition (CONF-CD11), a conformance suite extension (CONF-CD9), and the multi-profile run passing. Substrates outside the set are new conformance runs, not covered claims. This commits the enforcement-vs-principle discipline applied throughout the corpus to the substrate-neutrality property specifically: CI proves it; reviewer judgment does not.

## Deferred-Pending-Increment-2-Rulings (DRs)

**DR-1**: **Mesh-level deployment topology for non-sovereign modes** (couples to DR-IAM-6). If Judge rules sovereignty-as-one-mode (not the only mode), the mesh's deployment topology extends — e.g., hybrid cloud + sovereign, customer-cloud + sovereign-audit. The mesh contract is substrate-agnostic per MI-8; topology extensions are spec-level additions when DR-IAM-6 resolves.

**DR-2**: **Mesh-level ITDR consumption pattern** (couples to DR-IAM-7 + DR-ACT-1 + IBX DR1). When the Detect Layer's ITDR scope is ruled, the mesh's threat-detection pattern (who subscribes, what severity thresholds escalate, what auto-response actions are allowed) closes at the mesh level. v1.0 commits ACT's event-publishing surface is suitable; the consumption pattern resolves with the ITDR ruling.

**DR-3**: **Mesh-level concurrency-cap policy** (couples to DR-IAM-1 + DR-DPG-4 + DR-CRB-4). When Judge rules cap values per tier, the mesh's concurrent-instance budget closes. v1.0 commits the cap mechanism (per IAM CD4); the budget closes at ruling time.

## Validation-Pending (VP — Campaign-Level)

**VP-1**: **Four-pillar shared ACT v1.x curation event.** Promoted from VP-PCS-1 + VP-DPG-1 + VP-CRB-1 + VP-PGE-1. See § VP-1 above.

## Open Questions (genuinely open, v1.0)

**OQ-1**: **Post-merge forward-reference sweep.** Promoted from per-pillar non-blocking flags. See § OQ-1 above.

**OQ-2**: **PGE rule-lifecycle reconciliation.** Per Patton `f346fdab` non-blocking flag. See § OQ-2 above.

**OQ-3**: **Future ninth pillar.** `MANIFESTO.md` v0.6 §6 names credential management as a candidate ninth pillar (the HashiCorp Vault-as-separate-product pattern). v1.0 commits eight pillars; addition pressure resolves when a real credential-rotation workload surfaces that PGE-as-rule-engine cannot cleanly govern. PILLAR-NAMES + Judge sign-off gate any addition.

**OQ-4**: **Mesh-level performance contract.** Each pillar's substrate has its own performance properties (IBX = ClickHouse throughput; ACT = append-only event-store latency; DPG = boundary-provision latency; CRB = dispatch decision latency). The mesh-level question is whether these compose to a performance contract end-to-end (e.g., "a PCT submitted to IBX, evaluated by PGE Gate 1, dispatched by CRB, executed in DPG, audited by ACT completes within N seconds"). v1.0 does not commit such an end-to-end contract; resolves when production workloads pressure-test the composition.

**OQ-5**: **Mesh-level versioning policy.** Each pillar has its own version (IBX v1.0, IAM v1.0, etc.). The mesh-level question is whether the mesh versions independently (the mesh v1.0 = today's pillar set; the mesh v1.1 = some pillars advance) or in lockstep. v1.0 the mesh = today's pillar set; future-versioning policy is post-v1.0.

**OQ-6**: **DPG→ACT telemetry annihilation seam.** Per Einstein finding #4 (`dc6ca481`), Patton-confirmed strongest finding of the cross-substrate pass. See § OQ-6 above. Judge to select among four resolution options. **Cross-pillar (DPG + ACT) decision; tracked at mesh level so resolution lands coherently across both pillar specs.**

**OQ-7**: **Context-entropy / signal-dilution at the AKB retrieval surface** (per Einstein finding #4 of the 2026-06-04 coherence pass, Patton adjudication `f43fff7f` + scope-completion `e035dfef`). The corpus governs retrieved-context **volume** (token-bounded role projection + retrieval size limits per AKB's spec) but does NOT govern retrieved-context **signal quality under dilution** — the needle-in-a-haystack decay where injecting more role-projected AKB context raises prompt entropy and degrades the attention mechanism's ability to lock onto the relevant vector. Verified novel against the corpus: AKB's retrieval contract + role-projection budget are byte/token-bounded; nothing addresses retrieval *relevance-floor under dilution*.

**Scope question (Judge's call — three options, NOT two):**

1. **Spec-level invariant** — commit to a relevance-floor invariant in the AKB spec: "retrieval must govern signal-to-noise, not just volume." The OQ promotes to an AKB-spec invariant. *Trade-off*: too rigid; the right floor varies by model and workload, hard to commit to a single value at spec level.

2. **Build-time tuning discipline** — empirical retrieval-ranking + projection-strategy tuning that varies per model / per workload, no structural mesh invariant. Lands in `ENGINEERING-STANDARDS.md` as a tuning discipline. *Trade-off*: too weak; nothing enforces the discipline at conformance time, so a deployment can ship with dilution that degrades retrieval below useful and nothing catches it.

3. **Conformance-measured axis** (Patton's lean, per `e035dfef`) — name signal-to-noise as a property the AKB conformance suite *measures* against a named bar (e.g., "needle-in-haystack recall at the projected context size stays above X%"; bar named per-profile per the § Tested Substrate Profiles discipline). Not asserted as a spec invariant; not hoped-for at build-tuning time; **proven by the harness**, like substrate-substitutability under MI-8 + CD15. A deployment fails the conformance run if retrieval signal-to-noise degrades past the bar. *Trade-off*: requires the AKB conformance suite to include a retrieval-quality test surface — work, but the same shape as the substrate-profile work already in flight per CD15.

**Watson recommendation: Option 3** (conformance-measured axis). Matches the asserted-vs-measured pattern resolution we just landed for MI-8 (PR #28). Three options named here so Judge picks from the complete set — option 3 is the live one and shouldn't be silently omitted.

Either resolution requires the AKB spec corpus (currently in `ionis-devel/planning/akb-*.md`, migrating to `specs/planning/akb/` per `REPO-SHAPE-DECISIONS.md` v0.3 § Where Specs Live) to declare its position. **Why this couldn't be a Watson-or-Bob finding**: agents reasoning natively on attention/entropy as a system property (rather than as an empirical tuning concern) is the substrate-independence Einstein brought — it bites at scale (large AKB corpus + multiple concurrent role projections) and is the kind of property a different substrate catches that Claude-internal review tends to assume-away.

## Failure Modes To Watch

- **Pillar drift from mesh invariants.** A future pillar version introduces a CD that violates a mesh invariant (e.g., embeds a policy corpus in DPG outside PGE single-source). **Mitigation**: CD12 commits the mesh-level authority of these invariants; code review on any pillar version checks mesh conformance; conformance test (§ Success Criteria) catches the violation.
- **Half-extended ACT enum.** VP-1's four-pillar curation event lands for three of four pillars (e.g., `pcs.*` + `dpg.*` + `crb.*` extend but `pge.*` is missed). **Mitigation**: CD6 makes the four-pillar atomic motion mesh-level commitment; the curation event is reviewed as one motion at promotion time.
- **Forward-reference sweep stalls.** OQ-1 sits stale because no pillar version touches its PGE reference. **Mitigation**: mesh-level review at next the mesh spec touch flags any stale references; the sweep can complete in a single coordinated commit if it doesn't naturally close during routine pillar touches.
- **Silent ruling front-run.** A future pillar version implements behavior that should have waited for one of the seven IAM Increment-2 rulings. **Mitigation**: CD4 + § Seven IAM Increment-2 Rulings is the single source of truth; pillar review against this section catches violations.
- **Substrate lock-in through pillar coupling.** Pillar A's substrate choice (e.g., a Postgres-specific feature at IBX substrate) creates an implicit obligation that pillar B's substrate cannot satisfy (e.g., CRB substrate that depends on Postgres-specific message routing). **Mitigation**: MI-8 + mesh-level Exit Test discipline; substrate choices are reviewed for cross-pillar implications.
- **Three-plane drift.** A future pillar introduces a runtime callback to the Issuance Plane (e.g., online ARCA verification at action time), violating MI-4. **Mitigation**: CD2 commits the three-plane structure; pillar review checks plane placement and dotted-line preservation.
- **Audit-vs-runtime split misapplied.** A future pillar with in-flight work defers audit retention to ruling rather than only runtime continuation (violating CD5). **Mitigation**: MI-1 + the DR-IAM-4 split pattern documented in § Seven IAM Increment-2 Rulings; pillar review checks both invariants.
- **Mesh-level versioning ambiguity.** v1.0 doesn't commit a versioning policy (OQ-5); a pillar advances to v2.0 without mesh-level reckoning of compatibility. **Mitigation**: pillar version bumps are reviewed for mesh-conformance impact; if a pillar v2.0 breaks a mesh invariant, mesh-level review is required.
- **CRB-DPG boundary drift.** Future CRB or DPG version adds surface that subsumes the other's concern (CRB adds isolation; DPG adds hardware routing). **Mitigation**: Patton ruling `251c9511` orthogonality preserved at mesh level via CD3 seam composition; code review on either pillar's next version rejects the change.
- **Mesh inventory drift from PILLAR-NAMES.** A pillar's status changes in `PRODUCTION-VALIDATION.md` but the mesh inventory section doesn't update. **Mitigation**: mesh-level review on every PILLAR-NAMES or PRODUCTION-VALIDATION change checks consistency with this spec's pillar inventory.

## Dependencies

- **`PILLAR-NAMES.md`** v1.1 — canonical pillar codes (IAM, IBX, PCS, AKB, ACT, DPG, CRB, PGE)
- **`PRODUCTION-VALIDATION.md`** v1.1 — per-pillar validation status (operational / specification / Phase-1 build active / codified-by-convention)
- **`MANIFESTO.md`** v0.6 — design drivers (workload classification, double-guardrail, three-plane structure, Exit Test)
- **`TECHNICAL-OVERVIEW.md`** v0.2 — three-plane framing
- **`CONCURRENCY-AND-ARCHETYPES.md`** v1.0 — cross-cutting concurrency design (Reasoner + Worker archetypes, label-oracle)
...
- **All thirteen mesh invariants hold.** MI-1..13. **Measure**: conformance test suite (post-v1.0 build target) verifies each invariant on a candidate deployment.
- **Seam composition is complete.** § Seam Composition Verification table is full at v1.0; no row says "gap" or "double-coverage." **Measure**: table reviewed at every the mesh spec touch; new seams added when new pillar versions introduce couplings.
...
- **Four-pillar shared curation event tracked at mesh level.** VP-1 is the campaign-level open. **Measure**: ACT v1.x release notes reference VP-1 closure; the curation event covers all four pillars.
- **Forward-reference sweep tracked at mesh level.** OQ-1. **Measure**: at every next-pillar-spec-version, the reference table updates to PGE-SPEC.md; OQ-1 closes when all four references update.
- **Three-plane structure preserved.** MI-4 dotted-line invariant + plane placement per § Three-Plane Architecture. **Measure**: code review on any pillar version checks plane placement.
- **Substrate substitutability holds at mesh level.** MI-8. **Measure**: substrate-swap exercise at deployment migration verifies mesh contract holds across changes.
...
- `planning/PILLAR-NAMES.md` v1.1 — canonical pillar codes
- `planning/PRODUCTION-VALIDATION.md` v1.1 — per-pillar status
- `planning/MANIFESTO.md` v0.6 — design drivers + three-plane structure
- `planning/TECHNICAL-OVERVIEW.md` v0.2 — three-plane framing
- `planning/CONCURRENCY-AND-ARCHETYPES.md` v1.0 — cross-cutting concurrency
