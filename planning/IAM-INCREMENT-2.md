...
**IAM-INC2-CD12** (added v0.4, ratifies DR-IAM-1): **Concurrency cap defaults to 10 instances per identity, configurable per deployment via PGE policy + MCC control-plane override.** The Roster `status=active` + cost-center attribute provide the substrate; the cap policy evaluates against them. The cap is a single substrate value with optional per-tier variation supplied by configuration; the spec does not encode tier-specific defaults beyond the global 10.
...
**IAM-INC2-CD15** (added v0.4, ratifies DR-IAM-4): **Terminator action terminates the session(s), not the identity.** Identity persists across terminator actions; what is killed is the in-flight execution. On terminator invocation, all sessions for the affected identity transition to a terminal audit state per IAM-CORE-SPEC v1.0 SOM-CD5 (audit invariant). The agent retains its identity record (Roster `status` may be `suspended` per §D, but the record is intact for re-activation). Partial-failure handling on terminator invocation defaults to retry-with-bounded-attempts; specifics permitted as build surfaces them, with no further Judge ruling required. Per Judge: "you're not killing the agent, just what the agent's doing."
...
|---|---|---|---|
| **DR-IAM-1** (concurrency cap values per tier) | **Ruled** | CD12 | Default 10 per identity, configurable via PGE/MCC. |
...
| **DR-IAM-4** (terminator failure-mode + total-flood scope) | **Ruled** | CD15 | Terminator kills the session(s), not the identity. Identity persists. Partial-failure handling: retry-with-bounded-attempts default, latitude for build to refine. |
